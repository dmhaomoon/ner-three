...
def strage_combined_link_org_loc(tokens, tags):
    ...
    def string_output(data):
        line = []
        for i in data:
            a = (i.start,i.end,i.types,i.word)
            line.append(a)
        return line
    ...
    person, loc, org,accountant,time,num,per,bi,others = eval.get_result(tokens, tags)
    ...
    person_list.extend(string_output(accountant))
    person_list.extend(string_output(time))
    person_list.extend(string_output(num))
    person_list.extend(string_output(per))
    person_list.extend(string_output(bi))
    person_list.extend(string_output(others))
    return person_list
...
class Result(object):
    ...
    def get_result(self, tokens, tags, config=None):
        ...
        return self.person, self.loc, self.org,self.accountant,self.time,self.num,self.per,self.bi,self.others
    ...
    def append(self, word, start, end, tag):
        if tag == 'LOC':
            ...
            self.accountant.append(Pair(word, start, end, 'ACCOUNTANT'))
        elif tag == 'TIMI':
            self.time.append(Pair(word, start, end, 'TIMI'))
        elif tag == 'NUMBIR':
            self.num.append(Pair(word, start, end, 'NUMBIR'))
        elif tag == 'BI':
            self.bi.append(Pair(word, start, end, 'BI'))
        elif tag == 'PIRBON':
            self.per.append(Pair(word, start, end, 'PIRBON'))
        else:
            self.others.append(Pair(word, start, end, tag))


if __name__ == "__main__":
    sentence = '刚开增值税普通发票时，把发票号码弄错了怎么办？打印出来的号和实际发票上的号不符，怎么办？'
    predict(sentence)
    def convert(line):
        feature = convert_single_example(0, line, label_list, FLAGS.max_seq_length, tokenizer, 'p')
        input_ids = np.reshape([feature.input_ids],(batch_size, FLAGS.max_seq_length))
    ...
    file_write = open('./question_entity_result1.txt', mode='a', encoding='utf-8')
    ...
    with graph.as_default():
        print(id2label)
        start = datetime.now()
        for i in tqdm(range(int(len(sentences_lsit)/8))):
            sentence = sentences_lsit[i*8]
            sentence = tokenizer.tokenize(sentence)
            # print('your input is:{}'.format(sentence))
            input_ids, input_mask, segment_ids, label_ids = convert(sentence)
            ...
            file_write.write(sentences_lsit[i * 8])
            file_write.write(sentences_lsit[i * 8 + 1])
            file_write.write(sentences_lsit[i * 8 + 2])
            file_write.write(sentences_lsit[i * 8 + 3])
            file_write.write(sentences_lsit[i * 8 + 4])
            file_write.write(sentences_lsit[i * 8 + 5])
            file_write.write('bert_lstm_crf_book:' + str([result])+'\n')
            file_write.write(sentences_lsit[i * 8 + 6])
            file_write.write(sentences[i * 8 + 7])
