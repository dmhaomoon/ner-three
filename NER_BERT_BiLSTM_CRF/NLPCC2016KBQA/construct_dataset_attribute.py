...
target = "q_t_a_df_"+data_type+".csv"

attribute_classify_sample = []

# count the number of attribute
testing_df = pd.read_csv(target)
...
if data_type == 'testing':
    with open("attribute_"+data_type+"_sample.txt", "w") as f:
        f.write("\n".join(seq_result))
else:
    val_seq_result = seq_result[0:12000]
    with open("attribute_"+"val"+"_sample.txt", "w") as f:
        f.write("\n".join(val_seq_result))
